...
from src.auth import csrf_protected, generate_api_key, hash_api_key, login_required, user_is_expired
...
def dashboard():
    ...
    logs = db.execute(
        "SELECT * FROM request_logs WHERE user_id=? ORDER BY id DESC LIMIT 20", (g.user["id"],)
    ).fetchall()
